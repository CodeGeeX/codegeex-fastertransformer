...
class Code13BDictionary(object):
    ...
    def decode_tokens(self, tokens):
        decoded = ["50256" if token == 50256 else self.string(token) for token in tokens]
...
class CodeGeeXTokenizer(object):
    def __init__(
        ...
        if mode not in ['codegeex-13b', 'codegeex-python-13b']:
            raise ValueError(f"Invalid mode {mode}, choose from ['codegeex-13b', 'codegeex-python-13b']")
        self.start_extra_id = start_extra_id
        self.max_len = max_len
        self.mode = mode
        if dict_file is not None:
            self.code_dict = Code13BDictionary(dict_file, pad_to_vocab_size=51200) if self.mode == 'codegeex-python-13b' else None
        else:
            self.code_dict = None
