...
template<typename T>
void generate_codegeex_gemm_config(int batch_size,
                              int beam_width,
                              int max_input_len,
                              int head_num,
                              int size_per_head,
                              ...
                              bool isAppend)
{
    ...
    for (int i = 0; i < gemm_num; ++i) {
        ...
        if (data_type != FLOAT_DATATYPE && i != 1 && i != 2 && i != 10) {
            ...
            // for codegeex, computeType & scaleType should be FP32
    ...
    printf("***CODEGEEX Gemm Testing End***\n");
    return;
}

template void generate_codegeex_gemm_config<float>(int batch_size,
                                              int beam_width,
                                              int max_input_len,
                                              int head_num,
                                              int size_per_head,
...
template void generate_codegeex_gemm_config<half>(int batch_size,
                                             int beam_width,
                                             int max_input_len,
                                             int head_num,
                                             int size_per_head,
...
#ifdef ENABLE_BF16
template void generate_codegeex_gemm_config<__nv_bfloat16>(int batch_size,
                                                      int beam_width,
                                                      int max_input_len,
                                                      int head_num,
                                                      int size_per_head,
...
size_t calCodegeexGemmTestBufSizeInByte(int batch_size,
                                   int beam_width,
                                   int max_input_len,
                                   int head_num,
                                   int size_per_head,
