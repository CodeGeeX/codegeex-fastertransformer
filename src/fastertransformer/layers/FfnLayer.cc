...
template<typename T>
FastGeluFfnLayer<T>::FastGeluFfnLayer(size_t max_batch_size,
                              size_t max_seq_len,
                              size_t head_num,
                              size_t size_per_head,
                              size_t inter_size,
...
template<typename T>
FastGeluFfnLayer<T>::FastGeluFfnLayer(FastGeluFfnLayer<T> const& gelu_ffn_layer): FfnLayer<T>(gelu_ffn_layer)
{
}

template<typename T>
void FastGeluFfnLayer<T>::invokeAddBiasActivation(const int m, const T* bias)
{
    invokeAddBiasFastGelu<T>(inter_buf_, bias, m, inter_size_, stream_);
}

template class FastGeluFfnLayer<float>;
template class FastGeluFfnLayer<half>;
#ifdef ENABLE_BF16
template class FastGeluFfnLayer<__nv_bfloat16>;
#endif
