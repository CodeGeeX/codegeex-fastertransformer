                                        ...
                                        cudaStream_t stream)
{
    ...
    if (q_bias != nullptr) {
        ...
        params.k_bias = reinterpret_cast<const DataType*>(kv_bias)  ;
        params.v_bias = reinterpret_cast<const DataType*>(kv_bias) + 1 * hidden_units;
    }
    else {
...
template<typename T>
void DecoderTopSelfAttentionLayer<T>::allocateBuffer()
{
    if (is_allocate_buffer_ == false) {
        q_buf_ =
            reinterpret_cast<T*>(allocator_->malloc(sizeof(T) * max_batch_size_ * 1 * local_hidden_units_, false));
        kv_buf_ =
...
template<typename T>
void DecoderTopSelfAttentionLayer<T>::allocateBuffer(size_t batch_size)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    q_buf_ =
        reinterpret_cast<T*>(allocator_->reMalloc(q_buf_, sizeof(T) * batch_size * 1 * local_hidden_units_, false));
    kv_buf_ =
...
template<typename T>
void DecoderTopSelfAttentionLayer<T>::freeBuffer()
{
...
template<typename T>
bool DecoderTopSelfAttentionLayer<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
DecoderTopSelfAttentionLayer<T>::DecoderTopSelfAttentionLayer(size_t max_batch_size,
                                                        size_t head_num,
...
DecoderTopSelfAttentionLayer<T>::DecoderTopSelfAttentionLayer(size_t max_batch_size,
                                                        ...
                                                        bool sparse,
                                                        int int8_mode):
    DecoderTopSelfAttentionLayer<T>(max_batch_size,
                                 head_num,
                                 size_per_head,
                                 head_num,
                                 0,
...
template<typename T>
DecoderTopSelfAttentionLayer<T>::DecoderTopSelfAttentionLayer(DecoderTopSelfAttentionLayer<T> const& attention_layer):
    DecoderTopSelfAttentionLayer<T>(attention_layer.max_batch_size_,
...
template<typename T>
DecoderTopSelfAttentionLayer<T>::~DecoderTopSelfAttentionLayer()
{
...
template<typename T>
void DecoderTopSelfAttentionLayer<T>::forward(std::vector<fastertransformer::Tensor>* output_tensors,
                                           ...
                                           const AttentionWeight<T>* attention_weights)
{
    ...
    const T* query_input = reinterpret_cast<const T*>(input_tensors->at(6).data);
    ...
    if (sparse_ && cublas_wrapper_->isUseSparse(1, 2 * local_hidden_units_, m_padded, d_model_)) {
        ...
        if (int8_mode_ != 0 && batch_size <= 2) {
            ...
            int8WeightPerChannelLdkMultiplicationLauncher(attention_weights->key_weight.int8_kernel,
                                                          attention_input,
                                                          attention_weights->key_weight.quant_scale,
                                                        //   attention_weights->key_weight.scale,
                                                          kv_buf_,
                                                          batch_size,
                                                          2 * local_hidden_units_,
                                                          d_model_,
            ...
            if (int8_mode_ == 1) {
                FT_LOG_WARNING(
                    "[DecoderTopSelfAttentionLayer<T>::forward] int8 gpt doesn't support m > 2, run fp gpt instead.\n");
            }
            invokeMixWeightGemm(
...
template class DecoderTopSelfAttentionLayer<float>;
template class DecoderTopSelfAttentionLayer<half>;
#ifdef ENABLE_BF16
template class DecoderTopSelfAttentionLayer<__nv_bfloat16>;
#endif
