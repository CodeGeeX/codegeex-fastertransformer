...
#include "src/fastertransformer/layers/attention_layers/CodegeexTopContextAttentionLayer.h"
#include "src/fastertransformer/kernels/unfused_attention_kernels.h"

namespace fastertransformer {

template<typename T>
void CodegeexTopContextAttentionLayer<T>::forward(std::vector<fastertransformer::Tensor>* output_tensors,
                                          ...
                                          const AttentionWeight<T>* attention_weights)
{
    ...
    const T* query_input = (const T*)input_tensors->at(3).data;

    const int m = input_tensors->at(0).shape[0];

#ifdef SPARSITY_ENABLED
    const int m_padded = 8 * div_up(m, 8);
    if (sparse_ && cublas_wrapper_->isUseSparse(1, 2 * local_hidden_units_, m_padded, hidden_units_)) {
...
#ifdef SPARSITY_ENABLED
    const int m_padded = 8 * div_up(m, 8);
    if (sparse_ && cublas_wrapper_->isUseSparse(1, 1 * local_hidden_units_, m_padded, hidden_units_)) {
        cublas_wrapper_->SpGemm(CUBLAS_OP_N,
                                CUBLAS_OP_N,
...
#endif
    sync_check_cuda_error();
    invokeAddFusedTopQKVBiasTranspose(q_buf_2_,
                                   k_buf_2_,
                                   v_buf_2_,
                                   q_buf_1_,
                                   kv_buf_1_,
...
template<typename T>
CodegeexTopContextAttentionLayer<T>::CodegeexTopContextAttentionLayer(size_t max_batch_size,
                                                      size_t max_seq_len,
                                                      size_t head_num,
...
template<typename T>
CodegeexTopContextAttentionLayer<T>::CodegeexTopContextAttentionLayer(CodegeexTopContextAttentionLayer<T> const& attention_layer):
...
template<typename T>
CodegeexTopContextAttentionLayer<T>::~CodegeexTopContextAttentionLayer()
{
...
template<typename T>
void CodegeexTopContextAttentionLayer<T>::allocateBuffer()
{
    FT_CHECK(false);
    if (is_allocate_buffer_ == false) {
        q_buf_1_ = (T*)allocator_->malloc(sizeof(T) * 1 * max_batch_size_ * max_seq_len_ * local_hidden_units_, true);
        kv_buf_1_ = (T*)allocator_->malloc(sizeof(T) * 2 * max_batch_size_ * max_seq_len_ * local_hidden_units_, true);
...
template<typename T>
void CodegeexTopContextAttentionLayer<T>::allocateBuffer(size_t batch_size, size_t seq_len)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    q_buf_1_ = (T*)allocator_->reMalloc(q_buf_1_, sizeof(T) * 1 * batch_size * seq_len * local_hidden_units_, true);
    kv_buf_1_ = (T*)allocator_->reMalloc(kv_buf_1_, sizeof(T) * 2 * batch_size * seq_len * local_hidden_units_, true);
...
template<typename T>
void CodegeexTopContextAttentionLayer<T>::freeBuffer()
{
    if (is_allocate_buffer_) {
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
        allocator_->free(q_buf_1_);
        allocator_->free(kv_buf_1_);
...
template<typename T>
bool CodegeexTopContextAttentionLayer<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
bool CodegeexTopContextAttentionLayer<T>::isValidSeqLen(size_t seq_len)
{
...
template class CodegeexTopContextAttentionLayer<float>;
template class CodegeexTopContextAttentionLayer<half>;
#ifdef ENABLE_BF16
template class CodegeexTopContextAttentionLayer<__nv_bfloat16>;
#endif
