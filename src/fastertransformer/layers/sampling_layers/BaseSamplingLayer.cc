...
void BaseSamplingLayer<T>::freeBuffer()
{
    ...
    if (is_allocate_buffer_) {
        ...
        allocator_->free(random_seeds_buf_);
        allocator_->free(temperature_buf_);
        allocator_->free(repetition_penalty_buf_);
        allocator_->free(min_lengths_buf_);
        allocator_->free(runtime_logits_buf_);
        allocator_->free(skip_decode_buf_);
        delete[] temperature_;
