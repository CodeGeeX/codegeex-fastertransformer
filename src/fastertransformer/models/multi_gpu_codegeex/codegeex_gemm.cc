...
#include "src/fastertransformer/utils/gemm_test/codegeex_gemm_func.h"
...
int main(int argc, char* argv[])
{
    if (argc != 9 && argc != 10) {
        printf(
            "[ERROR] codegeex_gemm batch_size beam_width max_input_len head_number size_per_head inter_size vocab_size data_type tensor_para_size\n");
        printf("e.g. ./bin/codegeex_gemm 8 4 32 96 128 49152 51200 1 8\n");
        return 0;
    }
    ...
    void* gemm_test_buf;
    size_t buf_size_in_byte = ft::calCodegeexGemmTestBufSizeInByte(batch_size,
                                                              beam_width,
                                                              max_input_len,
                                                              head_num,
                                                              size_per_head,
    ...
    if (data_type == ft::FLOAT_DATATYPE) {
        ft::generate_codegeex_gemm_config<float>(batch_size,
                                            beam_width,
                                            max_input_len,
                                            head_num,
                                            size_per_head,
        ...
        ft::generate_codegeex_gemm_config<half>(batch_size,
                                           beam_width,
                                           max_input_len,
                                           head_num,
                                           size_per_head,
        ...
        ft::generate_codegeex_gemm_config<__nv_bfloat16>(batch_size,
                                                    beam_width,
                                                    max_input_len,
                                                    head_num,
                                                    size_per_head,
