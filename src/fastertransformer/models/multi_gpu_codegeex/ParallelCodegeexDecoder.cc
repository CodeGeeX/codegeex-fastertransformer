...
#include "src/fastertransformer/models/multi_gpu_codegeex/ParallelCodegeexDecoder.h"

namespace fastertransformer {

template<typename T>
void ParallelCodegeexDecoder<T>::initialize()
{
...
template<typename T>
ParallelCodegeexDecoder<T>::ParallelCodegeexDecoder(size_t max_batch_size,
                                          size_t head_num,
...
template<typename T>
ParallelCodegeexDecoder<T>::ParallelCodegeexDecoder(ParallelCodegeexDecoder<T> const& decoder):
...
template<typename T>
void ParallelCodegeexDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
...
template<typename T>
void ParallelCodegeexDecoder<T>::allocateBuffer(size_t batch_size)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    ...
    normed_decoder_output_buf_ = reinterpret_cast<T*>(
        allocator_->reMalloc(normed_decoder_output_buf_, sizeof(T) * batch_size * hidden_units_, false));
...
template<typename T>
void ParallelCodegeexDecoder<T>::freeBuffer()
{
...
template<typename T>
bool ParallelCodegeexDecoder<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
bool ParallelCodegeexDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool ParallelCodegeexDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool ParallelCodegeexDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int ParallelCodegeexDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
ParallelCodegeexDecoder<T>::~ParallelCodegeexDecoder()
{
...
template<typename T>
void ParallelCodegeexDecoder<T>::forward(std::vector<Tensor>* output_tensors,
                                    ...
                                    const ParallelCodegeexWeight<T>* codegeex_weights)
{
    ...
    for (uint l = 0; l < num_layer_-1; l++) {
        ...
        ffn_layer_->forward(&ffn_output_tensors, &ffn_input_tensors, &codegeex_decoder_layer_weight->at(l)->ffn_weights);
    ...
    for (uint l = 0; l < 1; l++) {
        T* decoder_input = (T*)( normed_decoder_output_buf_);//decoder_layer_output_);
        ...
        std::vector<Tensor> ffn_output_tensors{
            Tensor{MEMORY_GPU, data_type, {local_batch_size, hidden_units_}, decoder_output}};
        ffn_layer_->forward(&ffn_output_tensors, &ffn_input_tensors, &codegeex_topquery_layer_weight->at(l)->ffn_weights);
        invokeAddBiasResidual(decoder_output,
                              self_attn_output_,
                              ...
                              stream_);
        sync_check_cuda_error();
        if (isLastLayerParallelId(num_layer_-1) == true && pipeline_para_.rank_ != pipeline_para_.world_size_ - 1
...
template class ParallelCodegeexDecoder<float>;
template class ParallelCodegeexDecoder<half>;
#ifdef ENABLE_BF16
template class ParallelCodegeexDecoder<__nv_bfloat16>;
#endif
