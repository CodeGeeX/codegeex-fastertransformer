...
#include "src/fastertransformer/models/multi_gpu_codegeex/ParallelCodegeexWeight.h"

namespace fastertransformer {

template<typename T>
ParallelCodegeexWeight<T>::ParallelCodegeexWeight(const int hidden_units,
...
template<typename T>
ParallelCodegeexWeight<T>::~ParallelCodegeexWeight()
{
    ...
    for (int i = 0; i < 1; i++) {
        delete topquery_layer_weights[i];
    }
}

template<typename T>
ParallelCodegeexWeight<T>::ParallelCodegeexWeight(const ParallelCodegeexWeight& other):
...
template<typename T>
ParallelCodegeexWeight<T>& ParallelCodegeexWeight<T>::operator=(const ParallelCodegeexWeight& other)
{
    hidden_units_ = other.hidden_units_;
    inter_size_ = other.inter_size_;
    num_layer_ = other.num_layer_;
...
template<typename T>
void ParallelCodegeexWeight<T>::setWeightPtr()
{
...
template<typename T>
void ParallelCodegeexWeight<T>::mallocWeights()
{
...
template<typename T>
void ParallelCodegeexWeight<T>::loadModel(std::string dir_path)
{
    FtCudaDataType model_file_type = getModelFileType(dir_path + "/config.ini", "codegeex");
    ...
    loadWeightFromBin<T>(weights_ptr[5], {max_seq_len_, hidden_units_}, dir_path + "/model.topwpe.bin", model_file_type);
    ...
    for (int l = 0; l < 1; l++) {
        if (isValidLayerParallelId(l)) {
            topquery_layer_weights[l]->loadModel(dir_path + "/model.topquerylayers." + std::to_string(l), model_file_type);
        }
    }
}

template<typename T>
bool ParallelCodegeexWeight<T>::isValidLayerParallelId(int l)
{
    int local_num_layer = (int)(ceil((num_layer_-1) * 1.0f / layer_para_size_));
    return l < num_layer_ && (l >= local_num_layer * layer_para_rank_)
           && (l < local_num_layer * (layer_para_rank_ + 1));
}

template<typename T>
void ParallelCodegeexWeight<T>::resizeLayer(const int num_layer, const int int8_mode)
{
    int8_mode_ = int8_mode;
    num_layer_ = num_layer;
    for (int l = 0; l < num_layer_-1; l++) {
...
template<typename T>
void ParallelCodegeexWeight<T>::compress_weights(cublasMMWrapper& cublas_wrapper)
{
    ...
    FT_CHECK(topquery_layer_weights.size() == static_cast<size_t>(1));
    for (int i = 0; i < 1; ++i) {
        if (isValidLayerParallelId(i)) {
...
template struct ParallelCodegeexWeight<float>;
template struct ParallelCodegeexWeight<half>;
#ifdef ENABLE_BF16
template struct ParallelCodegeexWeight<__nv_bfloat16>;
#endif
