...
template<typename T>
void ParallelCodegeex<T>::initialize()
{
    codegeex_context_decoder_ = new ParallelCodegeexContextDecoder<T>(0,
                                                            0,
                                                            head_num_,
                                                            size_per_head_,
                                                            inter_size_,
    ...
    codegeex_decoder_ = new ParallelCodegeexDecoder<T>(0,
                                             head_num_,
                                             size_per_head_,
                                             inter_size_,
                                             num_layer_,
...
template<typename T>
void ParallelCodegeex<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void ParallelCodegeex<T>::allocateBuffer(size_t batch_size,
                                    size_t beam_width,
                                    size_t max_seq_len,
                                    ...
                                    bool is_return_context_cum_log_probs)
{
    ...
    decoder_input_top_buf_ = (T*)(allocator_->reMalloc(decoder_input_top_buf_, sizeof(T) * batchxbeam * hidden_units_, false));
    decoder_output_buf_ =
    ...
    context_decoder_input_top_buf_ = (T*)(allocator_->reMalloc(
        context_decoder_input_top_buf_, sizeof(T) * batchxbeam * max_input_len * hidden_units_, false));
...
template<typename T>
void ParallelCodegeex<T>::freeBuffer()
{
    if (is_allocate_buffer_) {
        ...
        allocator_->free(decoder_input_top_buf_);
        ...
        allocator_->free(context_decoder_input_top_buf_);
...
template<typename T>
ParallelCodegeex<T>::ParallelCodegeex(size_t max_batch_size,
                            size_t max_seq_len,
                            size_t max_input_len,
                            size_t beam_width,
                            size_t head_num,
...
template<typename T>
ParallelCodegeex<T>::ParallelCodegeex(ParallelCodegeex<T> const& codegeex):
    BaseLayer(codegeex),
    head_num_(codegeex.head_num_),
    size_per_head_(codegeex.size_per_head_),
    inter_size_(codegeex.inter_size_),
    num_layer_(codegeex.num_layer_),
    vocab_size_(codegeex.vocab_size_),
    start_id_(codegeex.start_id_),
    end_id_(codegeex.end_id_),
    beam_search_diversity_rate_(codegeex.beam_search_diversity_rate_),
    hidden_units_(codegeex.hidden_units_),
    top_k_(codegeex.top_k_),
    top_p_(codegeex.top_p_),
    random_seed_(codegeex.random_seed_),
    temperature_(codegeex.temperature_),
    len_penalty_(codegeex.len_penalty_),
    repetition_penalty_(codegeex.repetition_penalty_),
    tensor_para_(codegeex.tensor_para_),
    pipeline_para_(codegeex.pipeline_para_),
    local_head_num_(codegeex.local_head_num_),
    vocab_size_padded_(codegeex.vocab_size_padded_),
    int8_mode_(codegeex.int8_mode_),
    custom_all_reduce_comm_(codegeex.custom_all_reduce_comm_),
    enable_custom_all_reduce_(codegeex.enable_custom_all_reduce_)
{
    initialize();
}

template<typename T>
ParallelCodegeex<T>::~ParallelCodegeex()
{
    delete codegeex_decoder_;
    delete codegeex_context_decoder_;
    delete dynamic_decode_layer_;
    freeBuffer();
}

template<typename T>
void ParallelCodegeex<T>::computeContextCumLogProbs(float* cum_log_probs,
...
template<typename T>
void ParallelCodegeex<T>::forward(std::vector<Tensor>* output_tensors,
                             ...
                             const ParallelCodegeexWeight<T>* codegeex_weights)
{
    ...
    forward(&output_tensors_map, &input_tensors_map, codegeex_weights);
}

template<typename T>
void ParallelCodegeex<T>::forward(std::unordered_map<std::string, Tensor>* output_tensors,
                             ...
                             const ParallelCodegeexWeight<T>* codegeex_weights)
{
    ...
    if (vocab_size_ == vocab_size_padded_) {
        padded_embedding_kernel_ptr_ = codegeex_weights->post_decoder_embedding.kernel;
    }
    else {
        cudaMemcpyAsync(padded_embedding_kernel_,
                        codegeex_weights->post_decoder_embedding.kernel,
    ...
    if (input_tensors->count("prefix_soft_prompt_embedding") || max_input_length >= 1) {
        invokeTileCodegeexInputs(tiled_input_ids_buf_,
        ...
        if (input_tensors->count("prefix_soft_prompt_embedding")) {
            ...
            param.embedding_table = codegeex_weights->pre_decoder_embedding_table;
            param.pos_table = codegeex_weights->position_encoding_table;
            ...
            invokeInputIdsEmbeddingLookupPosEncoding(context_decoder_input_buf_,
                                                     output_ids_buf_,
                                                     codegeex_weights->pre_decoder_embedding_table,
                                                     codegeex_weights->position_encoding_table,
                                                     tiled_input_ids_buf_,
                                                     1,
                                                     max_input_length,
                                                     max_input_length,
        ...
        invokeInputIdsLookupTopPosEncoding(context_decoder_input_top_buf_,
                                                     output_ids_buf_,
                                                     codegeex_weights->topquery_position_encoding_table,
                                                     tiled_input_ids_buf_,
                                                     1,
                                                     max_input_length,
                                                     max_input_length,
        ...
        std::vector<Tensor> decoder_input_tensors{
            ...
            Tensor{MEMORY_GPU,
                   data_type,
                   {batch_size * beam_width, (size_t)max_input_length, hidden_units_},
                   context_decoder_input_top_buf_}
	};
        ...
        codegeex_context_decoder_->forward(
            &decoder_output_tensors, &decoder_input_tensors, &codegeex_weights->decoder_layer_weights, &codegeex_weights->topquery_layer_weights, codegeex_weights);
            ...
            if (!(max_input_length > 1 && step == max_input_length)) {
                if (pipeline_para_.rank_ == 0) {
                    invokeEmbeddingLookupPosEncoding(decoder_input_buf_ + hidden_units_offset,
                                                     ...
                                                     0,
                                                     stream_);
                    invokeLookupPosEncoding(decoder_input_top_buf_ + hidden_units_offset,
                                                     codegeex_weights->topquery_position_encoding_table,
                                                     output_ids_buf_ + id_offset,
                                                     tiled_input_lengths_buf_ + id_offset,
                                                     local_batch_size * beam_width,
                ...
                std::vector<Tensor> decoder_input_tensors{
                    ...
                    Tensor{MEMORY_GPU,
                           data_type,
                           {local_batch_size * beam_width, hidden_units_},
                           decoder_input_top_buf_ + hidden_units_offset}
		    };
...
template<typename T>
size_t ParallelCodegeex<T>::getPipelineParallelRank()
{
    return pipeline_para_.rank_;
}

template<typename T>
size_t ParallelCodegeex<T>::getPipelineParallelSize()
{
    return pipeline_para_.world_size_;
}

template<typename T>
size_t ParallelCodegeex<T>::getTensorParallelRank()
{
    return tensor_para_.rank_;
}

template<typename T>
size_t ParallelCodegeex<T>::getTensorParallelSize()
{
    return tensor_para_.world_size_;
}

template<typename T>
bool* ParallelCodegeex<T>::getFinishBuffer()
{
    return finished_buf_;
}

template class ParallelCodegeex<float>;
template class ParallelCodegeex<half>;
#ifdef ENABLE_BF16
template class ParallelCodegeex<__nv_bfloat16>;
#endif
