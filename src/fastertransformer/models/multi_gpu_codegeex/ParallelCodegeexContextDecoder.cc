...
#include "src/fastertransformer/models/multi_gpu_codegeex/ParallelCodegeexContextDecoder.h"
#include "src/fastertransformer/kernels/codegeex_kernels.h"
// #include <iostream>

namespace fastertransformer {

template<typename T>
void ParallelCodegeexContextDecoder<T>::initialize()
{
    ...
    self_top_attention_layer_ = new TensorParallelCodegeexTopContextAttentionLayer<T>(max_batch_size_,
                                                                          max_seq_len_,
                                                                          head_num_,
                                                                          size_per_head_,
                                                                          tensor_para_,
                                                                          ...
                                                                          custom_all_reduce_comm_,
                                                                          enable_custom_all_reduce_);
    ffn_layer_ = new TensorParallelFastGeluFfnLayer<T>(max_batch_size_,
                                                   max_seq_len_,
                                                   head_num_,
                                                   size_per_head_,
                                                   inter_size_,
...
template<typename T>
void ParallelCodegeexContextDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
...
template<typename T>
void ParallelCodegeexContextDecoder<T>::allocateBuffer(size_t batch_size, size_t seq_len)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    ...
    normed_self_attn_output_ = decoder_normed_input_;  // reuse the buffer
    normed_decoder_output_buf_ = reinterpret_cast<T*>(
        allocator_->reMalloc(normed_decoder_output_buf_, sizeof(T) * batch_size * seq_len * hidden_units_, false));
...
template<typename T>
void ParallelCodegeexContextDecoder<T>::freeBuffer()
{
...
template<typename T>
bool ParallelCodegeexContextDecoder<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
bool ParallelCodegeexContextDecoder<T>::isValidSeqLen(size_t seq_len)
{
...
template<typename T>
bool ParallelCodegeexContextDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool ParallelCodegeexContextDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool ParallelCodegeexContextDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int ParallelCodegeexContextDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
ParallelCodegeexContextDecoder<T>::ParallelCodegeexContextDecoder(size_t max_batch_size,
                                                        size_t max_seq_len,
                                                        size_t head_num,
...
template<typename T>
ParallelCodegeexContextDecoder<T>::ParallelCodegeexContextDecoder(ParallelCodegeexContextDecoder<T> const& decoder):
...
template<typename T>
ParallelCodegeexContextDecoder<T>::~ParallelCodegeexContextDecoder()
{
    delete self_attention_layer_;
    delete self_top_attention_layer_;
    delete ffn_layer_;
    freeBuffer();
}

template<typename T>
void ParallelCodegeexContextDecoder<T>::forward(
    std::vector<Tensor>* output_tensors,
    const std::vector<Tensor>* input_tensors,
    const std::vector<ParallelCodegeexDecoderLayerWeight<T>*>* codegeex_decoder_layer_weight,
    const std::vector<ParallelCodegeexTopQueryLayerWeight<T>*>* codegeex_topquery_layer_weight,
    const ParallelCodegeexWeight<T>* codegeex_weights)
{
    // input tensors:
    ...
    for (uint ite = 0; ite < iteration_num; ite++) {
        for (uint l = 0; l < num_layer_-1; l++) {
            ...
            invokeGeneralLayerNorm(decoder_normed_input_,
                                   decoder_input,
                                   codegeex_decoder_layer_weight->at(l)->pre_layernorm_weights.gamma,
                                   codegeex_decoder_layer_weight->at(l)->pre_layernorm_weights.beta,
            ...
            self_attention_layer_->forward(&self_attention_output_tensors,
                                           ...
                                           &codegeex_decoder_layer_weight->at(l)->self_attention_weights);
            if (is_final == false) {
                invokeGeneralAddBiasResidualPreLayerNorm(
                    self_attn_output_,
                    normed_self_attn_output_,
                    decoder_input,
                    codegeex_decoder_layer_weight->at(l)->self_attn_layernorm_weights.gamma,
                    codegeex_decoder_layer_weight->at(l)->self_attn_layernorm_weights.beta,
                    codegeex_decoder_layer_weight->at(l)->self_attention_weights.attention_output_weight.bias,
                    local_batch_size * seq_len,
                    hidden_units_,
                    stream_);
                sync_check_cuda_error();
                ...
                ffn_layer_->forward(
                    &ffn_output_tensors, &ffn_input_tensors, &codegeex_decoder_layer_weight->at(l)->ffn_weights);
                invokeAddBiasResidual(decoder_output,
                                      self_attn_output_,
                                      codegeex_decoder_layer_weight->at(l)->ffn_weights.output_weight.bias,
                                      local_batch_size * seq_len,
                                      hidden_units_,
                                      stream_);
                sync_check_cuda_error();
        ...
        invokeGeneralLayerNorm(normed_decoder_output_buf_ ,
                                       decoder_layer_output_ ,
                                       codegeex_weights->post_decoder_layernorm.gamma,
                                       codegeex_weights->post_decoder_layernorm.beta,
                                       local_batch_size * seq_len,
                                       hidden_units_,
                                       stream_);
        sync_check_cuda_error();
	    ...
	    //begin top layer
            T* decoder_input =
                           normed_decoder_output_buf_;//decoder_layer_output_;
            T* decoder_input_top =
                (T*)(input_tensors->at(3).data) + (int)(ite * local_batch_size * seq_len * hidden_units_) ;
            T* decoder_output =
                    (T*)(output_tensors->at(0).data) + (int)(ite * local_batch_size * seq_len * hidden_units_) ;
            
            invokeGeneralLayerNorm(decoder_normed_input_,
                                   decoder_input,
                                   codegeex_topquery_layer_weight->at(l)->pre_layernorm_weights.gamma,
                                   codegeex_topquery_layer_weight->at(l)->pre_layernorm_weights.beta,
                                   local_batch_size * seq_len,
                                   hidden_units_,
                                   stream_);
            sync_check_cuda_error();
            std::vector<Tensor> self_attention_input_tensors{
                ...
                Tensor{MEMORY_GPU,
                       ...
                       (const T*)input_tensors->at(1).data + local_batch_size * ite * seq_len * seq_len},
                Tensor{MEMORY_CPU, TYPE_BOOL, {1}, &is_final},
                Tensor{MEMORY_GPU, data_type, {local_batch_size * seq_len, hidden_units_}, decoder_input_top}
	    };

            size_t cache_offset = num_layer_ - 1 - getFirstLayerParallelId();
            for (auto t = output_tensors->at(1).shape.begin() + 1; t != output_tensors->at(1).shape.end(); ++t) {
                cache_offset *= *t;
            };
            size_t ite_cache_offset = ite * local_batch_size;
            ...
            std::vector<Tensor> self_attention_output_tensors{
                ...
                Tensor{
                    MEMORY_GPU, data_type, self_v_cache_size, ((const T*)output_tensors->at(2).data) + cache_offset}};
            self_top_attention_layer_->forward(&self_attention_output_tensors,
                                           &self_attention_input_tensors,
                                           &codegeex_topquery_layer_weight->at(l)->self_attention_weights);
            ...
            if (is_final == false) {
                invokeGeneralAddBiasResidualPreLayerNorm(
                    ...
                    normed_self_attn_output_,
                    decoder_input,
                    codegeex_topquery_layer_weight->at(l)->self_attn_layernorm_weights.gamma,
                    codegeex_topquery_layer_weight->at(l)->self_attn_layernorm_weights.beta,
                    codegeex_topquery_layer_weight->at(l)->self_attention_weights.attention_output_weight.bias,
                    local_batch_size * seq_len,
                    hidden_units_,
                    stream_);
                sync_check_cuda_error();
                ...
                ffn_layer_->forward(
                    &ffn_output_tensors, &ffn_input_tensors, &codegeex_topquery_layer_weight->at(l)->ffn_weights);
                invokeAddBiasResidual(decoder_output,
                                      self_attn_output_,
                                      codegeex_topquery_layer_weight->at(l)->ffn_weights.output_weight.bias,
                                      local_batch_size * seq_len,
                                      hidden_units_,
                                      stream_);
                sync_check_cuda_error();
...
template class ParallelCodegeexContextDecoder<float>;
template class ParallelCodegeexContextDecoder<half>;
#ifdef ENABLE_BF16
template class ParallelCodegeexContextDecoder<__nv_bfloat16>;
#endif
