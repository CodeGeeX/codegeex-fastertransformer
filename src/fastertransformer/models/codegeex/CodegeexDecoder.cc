...
#include "src/fastertransformer/models/codegeex/CodegeexDecoder.h"

namespace fastertransformer {

template<typename T>
void CodegeexDecoder<T>::initialize()
{
    self_attention_layer_ = new DecoderSelfAttentionLayer<T>(max_batch_size_,
                                                             ...
                                                             sparse_);
    self_top_attention_layer_ = new DecoderTopSelfAttentionLayer<T>(max_batch_size_,
                                                             head_num_,
                                                             size_per_head_,
                                                             stream_,
                                                             cublas_wrapper_,
...
template<typename T>
void CodegeexDecoder<T>::allocateBuffer()
{
...
template<typename T>
void CodegeexDecoder<T>::freeBuffer()
{
...
template<typename T>
bool CodegeexDecoder<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
CodegeexDecoder<T>::CodegeexDecoder(size_t max_batch_size,
                          size_t head_num,
...
template<typename T>
CodegeexDecoder<T>::CodegeexDecoder(CodegeexDecoder<T> const& decoder):
...
template<typename T>
CodegeexDecoder<T>::~CodegeexDecoder()
{
...
template<typename T>
void CodegeexDecoder<T>::forward(std::vector<Tensor>* output_tensors,
                            const std::vector<Tensor>* input_tensors,
                            const std::vector<CodegeexDecoderLayerWeight<T>>* codegeex_decoder_layer_weight)
{
    // input tensors:
    ...
    for (uint l = 0; l < 1; l++) {
        const T* decoder_input = (const T*)(decoder_layer_output_);
        T* decoder_output = (T*)( output_tensors->at(0).data );

        size_t self_key_cache_offset = num_layer_-1;
        for (auto t = output_tensors->at(1).shape.begin() + 1; t != output_tensors->at(1).shape.end(); ++t) {
            self_key_cache_offset *= (*t);
        }
        size_t self_value_cache_offset = num_layer_-1;
        for (auto t = output_tensors->at(2).shape.begin() + 1; t != output_tensors->at(2).shape.end(); ++t) {
            self_value_cache_offset *= (*t);
        }
...
template class CodegeexDecoder<float>;
template class CodegeexDecoder<half>;
