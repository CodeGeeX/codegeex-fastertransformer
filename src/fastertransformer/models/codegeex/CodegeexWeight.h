...
template<typename T>
struct CodegeexWeight {

    CodegeexWeight() = default;
    CodegeexWeight(const int hidden_units,
              const int inter_size,
              const int vocab_size,
              const int num_layer,
              const int max_seq_len);
    ~CodegeexWeight();
    CodegeexWeight(const CodegeexWeight& other);
    CodegeexWeight& operator=(const CodegeexWeight& other);
    ...
    std::vector<CodegeexDecoderLayerWeight<T>> topquery_layer_weights;
    std::vector<CodegeexDecoderLayerWeight<T>> decoder_layer_weights;
