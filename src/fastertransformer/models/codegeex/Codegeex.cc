...
#include "src/fastertransformer/models/codegeex/Codegeex.h"
...
template<typename T>
void Codegeex<T>::initialize()
{
    codegeex_context_decoder_ = new CodegeexContextDecoder<T>(max_batch_size_ * beam_width_,
                                                    max_input_len_,
                                                    head_num_,
                                                    size_per_head_,
                                                    inter_size_,
    ...
    codegeex_decoder_ = new CodegeexDecoder<T>(max_batch_size_ * beam_width_,
                                     head_num_,
                                     size_per_head_,
                                     inter_size_,
                                     num_layer_,
...
template<typename T>
void Codegeex<T>::allocateBuffer()
{
    if (is_allocate_buffer_ == false) {
        ...
        context_decoder_top_input_buf_ =
            (T*)(allocator_->malloc(sizeof(T) * batchxbeam * max_input_len_ * hidden_units_, false));
...
void Codegeex<T>::freeBuffer()
{
    if (is_allocate_buffer_ == true) {
        ...
        allocator_->free(context_decoder_top_input_buf_);
...
template<typename T>
bool Codegeex<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
bool Codegeex<T>::isValidSeqLen(size_t seq_len)
{
...
template<typename T>
bool Codegeex<T>::isValidInputSeqLen(size_t seq_len)
{
...
template<typename T>
Codegeex<T>::Codegeex(size_t max_batch_size,
            size_t max_seq_len,
...
template<typename T>
Codegeex<T>::Codegeex(Codegeex<T> const& codegeex):
    BaseLayer(codegeex),
    max_batch_size_(codegeex.max_batch_size_),
    max_seq_len_(codegeex.max_seq_len_),
    max_input_len_(codegeex.max_input_len_),
    beam_width_(codegeex.beam_width_),
...
template<typename T>
void Codegeex<T>::forward(std::vector<Tensor>* output_tensors,
                     const std::vector<Tensor>* input_tensors,
                     const CodegeexWeight<T>* codegeex_weights)
{
    // input_tensors:
    ...
    if (max_input_length == 0) {
        ...
        invokeInputIdsEmbeddingLookupPosEncoding(context_decoder_input_buf_,
                                                 ...
                                                 hidden_units_,
                                                 stream_);
        invokeInputIdsLookupTopPosEncoding(context_decoder_top_input_buf_,
                                                 output_ids_buf_,
        ...
        codegeex_context_decoder_->forward(
            &decoder_output_tensors, &decoder_input_tensors, &codegeex_weights->decoder_layer_weights, &codegeex_weights->topquery_layer_weights);
    ...
    for (int step = max_input_length; step < (int)max_output_seq_len; step++) {
        ...
        invokeEmbeddingLookupPosEncoding(decoder_input_buf_,
                                         ...
                                         0,
                                         stream_);
        invokeLookupTopPosEncoding(decoder_top_input_buf_,
                                         codegeex_weights->topquery_position_encoding_table,
                                         output_ids_buf_,
                                         input_length_ptr,
                                         batch_size * beam_width_,
        ...
        std::vector<Tensor> decoder_input_tensors{
            ...
            Tensor{MEMORY_GPU, data_type, {batch_size * beam_width_, hidden_units_}, decoder_top_input_buf_}
	};
        std::vector<Tensor> decoder_output_tensors{
            Tensor{MEMORY_GPU, data_type, {batch_size * beam_width_, hidden_units_}, decoder_output_buf_},
            Tensor{MEMORY_GPU, data_type, self_k_cache_size, key_caches_[src_cache_id]},
            Tensor{MEMORY_GPU, data_type, self_v_cache_size, value_caches_[src_cache_id]}};
        codegeex_decoder_->forward(&decoder_output_tensors, &decoder_input_tensors, &codegeex_weights->decoder_layer_weights, &codegeex_weights->topquery_layer_weights);
...
template class Codegeex<float>;
template class Codegeex<half>;
