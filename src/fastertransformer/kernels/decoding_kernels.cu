...
template<typename T>
__global__ void posLookupPosEncoding(T* from_tensor,
                                           const T* position_encoding,
                                           const int* all_ids,
                                           const int* input_lengths,
                                           const int local_batch_size,
                                           ...
                                           const T scale)
{
    ...
    for (int index = blockIdx.x * blockDim.x + threadIdx.x; index < local_batch_size * hidden_units;
        ...
        const int step_offset = input_lengths == nullptr ?
                                    step * hidden_units :
                                    (step - max_input_length + input_lengths[row_index]) * hidden_units;
        T val = position_encoding[step_offset + col_index];
...
template<typename T>
void invokeLookupPosEncoding(T* from_tensor,
                                      const T* position_encoding,
                                      const int* all_ids,
                                      const int* input_lengths,
                                      const int local_batch_size,
                                      ...
                                      cudaStream_t stream)
{
    dim3 grid(min(local_batch_size, 65536));
    dim3 block(min(hidden_units, 1024));
    posLookupPosEncoding<T><<<grid, block, 0, stream>>>(from_tensor,
                                                                  position_encoding,
                                                                  all_ids,
                                                                  input_lengths,
                                                                  local_batch_size,
...
template void invokeLookupPosEncoding(float* from_tensor,
                                               const float* position_encoding,
                                               const int* all_ids,
                                               const int* input_lengths,
                                               const int local_batch_size,
...
template void invokeLookupPosEncoding(half* from_tensor,
                                               const half* position_encoding,
                                               const int* all_ids,
                                               const int* input_lengths,
                                               const int local_batch_size,
...
#ifdef ENABLE_BF16
template void invokeLookupPosEncoding(__nv_bfloat16* from_tensor,
                                               const __nv_bfloat16* position_encoding,
                                               const int* all_ids,
                                               const int* input_lengths,
                                               const int local_batch_size,
