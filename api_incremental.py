...
parser.add_argument('--output_len_incremental', type=int, default=1,
                    help='output sequence length to generate.')
...
parser.add_argument('--return_cum_log_probs', type=int, default=1, choices=[0, 1, 2],
...
output_len_incremental = args.output_len_incremental
...
def process_code(
    ...
    start_lengths_origin = start_lengths.clone()
        ...
        if not incremental:
            ...
            if return_cum_log_probs == 0:
                ...
                for i, (context, tokens, cum_log_probs) in enumerate(zip(contexts, tokens_batch[0], tokens_batch[2])):
                    tokens = tokens.cpu().detach().tolist()[0]
                    cum_log_probs = cum_log_probs.cpu().detach().tolist()[0]
                    end_idx = len(tokens)
                    print("token len: ", end_idx)
                    if tokens[start_lengths[i]] == tokenizer.eos_token_id:
                        outputs.append({"context": context, "generated": ""})
            ...
            is_finished = [False for _ in range(len(contexts))]
            for _ in range(output_len // output_len_incremental + 1):
                tokens_batch = codegeex(start_ids,
                                        start_lengths,
                                        output_len_incremental,
                                        beam_width,
                                        top_k,
                                        top_p,
                                        beam_search_diversity_rate,
